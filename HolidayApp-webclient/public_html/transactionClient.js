...
// Model for Holidaytransaction entity
    models.Holidaytransaction = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/HolidayApp-webservice/webresources/transaction/",
        idAttribute: 'transactionno',
        defaults: {
            name: "",
            description: "",
            transactiontype: "",
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/HolidayApp-webservice/webresources/transaction/';
            }
    ...
    // Collection class for Holidaytransaction entities
    models.HolidaytransactionCollection = Backbone.Collection.extend({
        model: models.Holidaytransaction,
        url: "http://localhost:8080/HolidayApp-webservice/webresources/transaction/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.HolidaytransactionCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-holidaytransaction-list-itemis template identifier for item
                        templateName: '#tpl-holidaytransaction-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.holidaytransaction = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.holidaytransaction,
                    // tpl-holidaytransaction-details is template identifier for chosen model element
                    templateName: '#tpl-holidaytransaction-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Holidaytransaction();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-holidaytransaction-details is a template identifier for chosen model element
                templateName: '#tpl-holidaytransaction-details',
