...
// Model for Product entity
    models.Product = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/HolidayApp-webservice/webresources/product/",
        idAttribute: 'id',
        defaults: {
            price: "",
            description: "",
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/HolidayApp-webservice/webresources/product/';
            }
    ...
    // Collection class for Product entities
    models.ProductCollection = Backbone.Collection.extend({
        model: models.Product,
        url: "http://localhost:8080/HolidayApp-webservice/webresources/product/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.ProductCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-product-list-itemis template identifier for item
                        templateName: '#tpl-product-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.product = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.product,
                    // tpl-product-details is template identifier for chosen model element
                    templateName: '#tpl-product-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Product();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-product-details is a template identifier for chosen model element
                templateName: '#tpl-product-details',
